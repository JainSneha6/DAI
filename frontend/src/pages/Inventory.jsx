...
import { Send, Package, Bot, User, Sparkles, BarChart3, Database, Loader2, Download } from 'lucide-react';
...
export default function InventoryChat() {
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      content:
        'Hello! I can help you with inventory optimization and replenishment planning. You can ask me to:\n\n• Generate schedules ("optimize inventory for next 30 periods")\n• Show EOQ values\n• Analyze plans\n• Get replenishment details\n\nWhat would you like to optimize?',
      timestamp: new Date()
    }
  ]);
  ...
  const fetchAvailablePlans = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/inventory/chat/plans');
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/inventory/chat', {
        method: 'POST',
      ...
      if (data.success) {
        const assistantMessage = {
          ...
          planName: data.plan_info?.model || 'latest'
        };
        setMessages((prev) => [...prev, assistantMessage]);
        
        // Set download URL if plan data available
        if (data.plan_data && data.plan_info) {
          const planName = data.plan_info.model || 'latest';
          setDownloadUrl(`/api/inventory/chat/download_plan/${encodeURIComponent(planName)}`);
        }
      } else {
        setMessages((prev) => [
          ...prev,
  ...
  const handleDownload = (e, planName) => {
    ...
    if (downloadUrl) {
      ...
      link.download = `inventory_plan_${planName}.csv`;
  ...
  const quickActions = [
    { label: 'Optimize next 10 periods', query: 'Optimize inventory for next 10 periods' },
    { label: 'Show EOQ values', query: 'What are the EOQ values?' },
    { label: 'Get replenishment schedule', query: 'Show the replenishment plan' },
    { label: 'Explain my inventory data', query: 'Tell me about the inventory data' }
  ];

  const ScheduleVisualization = ({ data, planName }) => {
    ...
    return (
      <div className="mt-4 p-4 bg-gradient-to-br from-white/5 to-white/[0.02] rounded-lg border border-white/10 backdrop-blur-xl">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h4 className="font-semibold text-slate-100">Replenishment Schedule Preview</h4>
          {downloadUrl && (
            <button
              onClick={(e) => handleDownload(e, planName)}
              className="ml-auto px-3 py-1 bg-green-600 hover:bg-green-700 text-white text-xs rounded font-medium flex items-center gap-1"
            >
        ...
        <div className="space-y-3 mb-4 max-h-60 overflow-y-auto">
          {skus.slice(0, 5).map((sku) => {
            const orders = schedule[sku] || {};
            if (Object.keys(orders).length === 0) return null;
            return (
              <div key={sku} className="bg-slate-800/30 p-3 rounded border border-slate-700">
                <div className="font-medium text-slate-200 mb-2">{sku}</div>
                <div className="space-y-1 text-sm">
                  {Object.entries(orders).slice(0, 3).map(([period, qty]) => (
                    <div key={period} className="flex justify-between">
                      <span className="text-slate-400">Period {period}</span>
                      <span className="font-semibold text-green-300">{qty}</span>
                    </div>
                  ))}
        ...
        {skus.length > 5 && (
          <div className="text-center text-xs text-slate-400 mb-3">
            Showing first 5 SKUs. Download full plan for all {skus.length} SKUs.
          </div>
        )}

        <div className="grid grid-cols-2 gap-3 text-sm">
          <div className="bg-gradient-to-br from-white/3 to-white/[0.02] p-2 rounded border border-white/6">
            <div className="text-slate-400 text-xs">SKUs</div>
            <div className="font-bold text-slate-100">{skus.length}</div>
          </div>
          <div className="bg-gradient-to-br from-white/3 to-white/[0.02] p-2 rounded border border-white/6">
            <div className="text-slate-400 text-xs">Orders</div>
            <div className="font-bold text-slate-100">{Object.values(schedule).reduce((acc, orders) => acc + Object.keys(orders).length, 0)}</div>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="mt-3 p-3 bg-gradient-to-br from-white/5 to-white/[0.02] rounded-lg border border-white/10 backdrop-blur-xl">
        ...
        <div className="space-y-1 text-xs text-slate-200">
          ...
          <div>
            <span className="font-medium text-slate-300">SKUs:</span> {info.skus?.length || 0}
          </div>
          {info.EOQ && Object.keys(info.EOQ).length > 0 && (
            <div>
              <span className="font-medium text-slate-300">Sample EOQ:</span> {Object.values(info.EOQ).slice(0, 1)[0]?.toFixed(2)}
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0f1724] via-[#111827] to-[#071029] relative overflow-hidden text-slate-100">
      ...
      <div className="absolute top-16 left-28 w-96 h-96 bg-green-600/10 rounded-full blur-[120px]" />
      ...
      <div className="relative z-10 max-w-5xl mx-auto flex flex-col h-screen">
        ...
        <div className="p-6 border-b border-white/6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-100">Inventory Optimization & Replenishment Assistant</h1>
              <p className="text-slate-400 text-sm">AI-powered planning & scheduling</p>
            </div>
          ...
          {messages.map((message, idx) => (
            <div key={idx} className={`flex gap-3 ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              >
                ...
                {message.planData && <ScheduleVisualization data={message.planData} planName={message.planName} />}
                {message.planInfo && <PlanInfoCard info={message.planInfo} />}
          ...
          <div className="flex gap-3 items-end">
            <div className="flex-1 relative">
              <textarea
                ...
                placeholder="Ask about optimization, schedules, EOQ, or your inventory..."
                disabled={isLoading}
                rows={1}
                className="w-full px-4 py-3 pr-12 border-2 border-transparent rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 transition-all resize-none bg-white/5 text-slate-100"
            ...
            <button
              ...
              className="px-6 py-3 my-2 bg-gradient-to-r from-green-600 to-emerald-700 hover:from-green-700 hover:to-emerald-800 text-white rounded-xl font-medium transition-all duration-200 disabled:from-gray-400 disabled:to-gray-500 disabled:cursor-not-allowed shadow-md flex items-center gap-2"
            >
