...
function VirtualConsultant() {
  ...
  const [initialLoading, setInitialLoading] = useState(true); // New state for initial data processing

  // This function will be called when the page loads
  useEffect(() => {
    const processAndStoreData = async () => {
      try {
        setInitialLoading(true); // Set loading state for initial data processing
        ...
        setInitialLoading(false); // Stop loading state after the request finishes
      }
    };

    processAndStoreData();
  }, []); // Empty dependency array to call this effect only once when the component mounts
      ...
      <div className="container mx-auto">
        ...
        <main className="flex-1 overflow-y-auto p-4">
          <div className="rounded-lg shadow-lg p-4 h-full overflow-auto">
            {/* Show initial loading while processing data */}
            {initialLoading ? (
